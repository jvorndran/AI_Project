...
class WarehouseSimulation:
    def __init__(self, grid_size=40, cell_size=20, num_robots=10, num_obstacles=50):
        pygame.init()
    ...
    def calculate_time_cost(self, robot, task):
        """
        Calculate the time cost for a robot to take on a new task, considering both
        the distance to the task and the handling time.
        """
        if robot.tasks:
            # Calculate the distance from the last delivery point to the new task's pickup point
            last_delivery_point = robot.tasks[-1][1]
            distance_to_new_task = np.linalg.norm(last_delivery_point - np.array(task[0]))
            # Add the handling time for the current tasks
            current_handling_time = len(robot.tasks) * TASK_HANDLING_TIME
        else:
            distance_to_new_task = np.linalg.norm(robot.current_pos - np.array(task[0]))
            current_handling_time = 0

        # Estimate travel time based on distance (assuming a constant speed)
        travel_time = distance_to_new_task / SPEED  # Define SPEED according to your simulation's scale

        return travel_time + current_handling_time + TASK_HANDLING_TIME
    ...
    def assign_tasks(self, num_tasks):
        ...
        for _ in range(num_tasks):
            ...
            task_color = self.generate_unique_color(used_colors)
            min_time_cost = float('inf')
            best_robot = None
            for robot in self.robots:
                time_cost = self.calculate_time_cost(robot, (pickup, delivery))
                if time_cost < min_time_cost:
                    min_time_cost = time_cost
                    best_robot = robot
            if best_robot is not None:
                best_robot.add_task([np.array(pickup), np.array(delivery), task_color])
            print(f'Assigned task to robot at position {best_robot.current_pos} with cost {min_time_cost}')
